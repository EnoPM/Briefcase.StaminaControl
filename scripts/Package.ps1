...
if($manifest.update.provider -cne 'github-releases' -or $manifest.update.repository -cne "EnoPM/$($config.repository)"){throw 'Invalid update repository'}
...
$rows=@(Get-ChildItem -LiteralPath (Join-Path $Stage 'Briefcase') -Recurse -File|ForEach-Object{
 ...
 if($relative -ceq "Briefcase/Mods/$($manifest.id)/Data/config.json"){$row.preserve=$true};$row
})
@{updateSchema=1;kind='briefcase-mod';platform='windows-x64';modId=$manifest.id;version=$manifest.version;repository=$manifest.update.repository;files=$rows}|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $Stage 'ModPackage.json') -Encoding utf8
$expected+=@('ModPackage.json')
